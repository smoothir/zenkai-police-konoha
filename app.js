...
function checkLogin() {
    ...
    else { document.getElementById('login-screen').style.display = 'none'; document.getElementById('agent-name').innerHTML = `<i class="fas fa-user-shield"></i> Agent : ${currentUser}`; }
}
...
function performLogin() {
    ...
    if (nom && prenom) {
        ...
        addAuditLog("Connexion approuvée.");
    } else { alert("Identification requise."); }
}

function logout() { addAuditLog("Déconnexion."); sessionStorage.removeItem('rpUser'); location.reload(); }
...
function openDossier(type, id) {
    ...
    if(type === 'bingo') html += `<div class="dossier-block" style="border-color:red; background:#2b1111;"><h4>Prime & Rang</h4><p>Classe ${item.rang} - <span style="color:#ffcc00;">${item.prime} ¥</span></p></div><div class="dossier-block"><h4>Directives</h4><p>${item.notes}</p></div>`;
    if(type === 'village') html += `<div class="dossier-block"><h4>Infos</h4><p><strong>Chef:</strong> ${item.chef}</p><p>${item.notes}</p></div>`;
    if(type === 'clan') html += `<div class="dossier-block"><h4>Infos</h4><p><strong>Chef:</strong> ${item.chef}</p><p><strong>Kekkei Genkai:</strong> ${item.heredite}</p></div><div class="dossier-block"><h4>Histoire</h4><p>${item.histoire}</p><p>${item.notes}</p></div>`;
    if(type === 'mission') {
